...
const SparkleNavbar = ({ items, color = "#8B4513" }) => {
  ...
  const getOffsetLeft = (element) => {
    ...
    // Center the underline beneath the nav item
    return (
      elementRect.left -
      navRect.left +
  ...
  const handleClick = (index) => {
    ...
    activeElement.style.transform = `translateX(${currentX}) rotateY(${direction === "before" ? "180deg" : "0deg"})`;

    const maxWidth = navElement.offsetWidth - 60;
    const targetWidth = spacing > maxWidth ? maxWidth : spacing;
    ...
    const animate = () => {
      ...
      if (animationPhase === 0 && elapsed < 300) {
        const progress = elapsed / 300;
        activeElement.style.setProperty('--active-element-width', `${targetWidth * progress}px`);
        activeElement.style.setProperty('--active-element-opacity', `${Math.min(progress * 10, 1)}`);
        
        if (elapsed < 100 && !activeElement.innerHTML) {
        ...
        animationPhase = 1;
        animate();
      } else if (animationPhase === 1 && elapsed < 600) {
        const progress = (elapsed - 300) / 300;
        activeElement.style.setProperty('--active-element-scale-x', `${1 - progress}`);
        activeElement.style.setProperty('--active-element-scale-y', `${1 - progress * 0.75}`);
        activeElement.style.setProperty('--active-element-width', `${targetWidth * (1 - progress)}px`);
        activeElement.style.setProperty('--active-element-mask-position', `${progress * 40}%`);
        
        if (elapsed > 450) {
          activeElement.style.setProperty('--active-element-opacity', `${1 - (elapsed - 450) / 150}`);
        }
        ...
        activeElement.style.transform = `translateX(${getOffsetLeft(newButton)}px)`;
        activeElement.style.setProperty('--active-element-show', '1');
        activeElement.style.setProperty('--active-element-scale-x', '1');
        activeElement.style.setProperty('--active-element-scale-y', '1');
        activeElement.style.setProperty('--active-element-opacity', '0');
        activeElement.style.setProperty('--active-element-width', '0px');
        activeElement.style.setProperty('--active-element-mask-position', '0%');
        setActiveIndex(index);
      }
    };

    const startX = getOffsetLeft(oldButton);
    const animatePosition = () => {
      ...
      if (elapsed < 600) {
        ...
        const currentTransform = activeElement.style.transform;
        const rotateMatch = currentTransform.match(/rotateY\([^)]+\)/);
        const rotateY = rotateMatch ? rotateMatch[0] : '';
        activeElement.style.transform = `translateX(${currentX}px) ${rotateY}`;
        activeElement.style.setProperty('--active-element-strike-x', `${-50 * progress}%`);
  ...
  return (
    <>
      <style>{`
        .sparkle-navigation-menu {
          position: relative;
          z-index: 1;
          padding-bottom: 12px;
        }

        .sparkle-navigation-menu ul {
          margin: 0;
          padding: 0;
          list-style: none;
          display: flex;
        ...
        .sparkle-navigation-menu .active-element > svg,
        .sparkle-navigation-menu .active-element .strike {
          position: absolute;
          left: 0;
          top: 50%;
        ...
        .sparkle-navigation-menu.after .active-element > svg,
        .sparkle-navigation-menu.after .active-element .strike {
          left: auto;
          right: 0;
        }
        ...
        .sparkle-navigation-menu.before .active-element > svg {
        ...
        .sparkle-navigation-menu .active-element .strike svg {
          display: block;
          overflow: visible;
          height: 12px;
        ...
        .sparkle-navigation-menu .active-element .strike svg:last-child {
          transform: translate(var(--active-element-strike-x), -30%) scale(-1);
        }

        .sparkle-navigation-menu .active-element .strike svg g path:nth-child(2) {
          filter: blur(2px);
        }

        .sparkle-navigation-menu .active-element .strike svg g path:nth-child(3) {
          filter: blur(4px);
        }
